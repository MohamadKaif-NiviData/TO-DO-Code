  ...
  if (event.key === "Enter") {
    ...
    if (input.value.length > 0) {
      document.querySelector("#tasks").innerHTML += `
        ...
        <br>
          <input type="checkbox" id ="myCheck${count_check_lbl_del}"  onClick="ck_checked(${count_check_lbl_del})"> 
      <label id ="lbclick${count_check_lbl_del}" onClick="lbclick(${count_check_lbl_del})" class="fw-bold" class="text-center ">  ${
    ...
    r.addEventListener("keypress", function (event) {
      if (event.key === "Enter") {
        ...
        if (box.value.length > 0) {
          document.getElementById(`lbclick${c}`).innerHTML = `${
            document.getElementById(`bx${c}`).value
          }`;
        } else {
